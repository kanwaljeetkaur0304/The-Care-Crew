...
import { X, Star, CheckCircle, FileText, Check, LogIn, UserPlus, ArrowLeft } from 'lucide-react';
import { Elements } from '@stripe/react-stripe-js';
import { useTheme } from '../context/ThemeContext';
import { useSubscription, LISTING_PLANS } from '../context/SubscriptionContext';
...
interface ListingSubscriptionModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function ListingSubscriptionModal({ isOpen, onClose }: ListingSubscriptionModalProps) {
  const { isDark } = useTheme();
  const { purchaseListingSubscription } = useSubscription();
  const { user } = useAuth();
  const { openAuthModal } = useUI();
  const [step, setStep] = useState<'plans' | 'auth' | 'payment' | 'success'>('plans');
  const [selectedPlan, setSelectedPlan] = useState('list-2m');
  ...
  const handlePaymentSuccess = () => {
    purchaseListingSubscription(selectedPlan);
    setStep('success');
    setError('');
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        className={`relative z-10 w-full max-w-4xl rounded-3xl border shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto ${
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h3 className={`font-display text-lg font-semibold ${isDark ? 'text-ink' : 'text-light-text'}`}>
              {step === 'plans' && 'Activate Your Freelance Listing'}
              {step === 'auth' && 'Sign In to Continue'}
              {step === 'payment' && 'Payment'}
              {step === 'success' && 'Listing Slot Active'}
            </h3>
          </div>
          <button
            onClick={handleClose}
          ...
          {step === 'plans' && (
            <div className="space-y-4">
              <p className={`text-sm mb-6 ${isDark ? 'text-ink-muted' : 'text-light-text-muted'}`}>
                Choose a plan to post your freelance listing. Both families and caregivers use the same listing subscription.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {LISTING_PLANS.map((p) => (
                  ...
                  >
                    ...
                    <div className="mb-6">
                      <div className="font-display text-3xl font-bold text-gold">${p.price}</div>
                      ...
                      {p.features.map((feature, idx) => (
                        <div key={idx} className="flex items-start gap-3">
                          {feature.included ? (
                            ...
                            <div
                              className={`flex-shrink-0 pt-0.5 w-4 h-4 rounded-full ${
                          ...
                          <span
                            className={`text-sm ${
                              feature.included
                                ...
                                : 'text-light-text-muted line-through'
                            }`}
                          >
                            {feature.name}
                          </span>
                    ...
                    <div
                      className={`mt-6 py-3 px-4 rounded-lg text-center text-sm font-medium transition-colors ${
                        selectedPlan === p.id
          ...
          {step === 'auth' && (
            <div className="space-y-6 py-2">
              <div className="text-center">
                ...
                >
                  <FileText className={`w-7 h-7 ${isDark ? 'text-gold' : 'text-maroon'}`} />
                </div>
                <p className={`text-sm ${isDark ? 'text-ink-muted' : 'text-light-text-muted'}`}>
                  You're one step away from activating your freelance listing. Sign in or create a free account to complete your purchase.
                </p>
              </div>
          ...
          {step === 'payment' && clientSecret && (
            <div className="space-y-5">
              ...
              <Elements stripe={stripePromise} options={{ clientSecret }}>
                <StripePaymentForm
                  amount={plan.price}
                  planLabel={`${plan.tier} Listing — ${plan.label}`}
