    ...
    <div className="space-y-6">
      <div>
        ...
        <p className={`text-sm mt-1 ${isDark ? 'text-ink-muted' : 'text-light-text-muted'}`}>
          Caregivers who want to connect with your family
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <DashboardEmptyState
          ...
          description="When caregivers apply to your job posts, they will appear here."
        />
      ) : (
          ...
          {filtered.map((req) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-medium transition-colors ${
                        isDark
                          ? 'border-red-400/20 text-red-400/60 hover:text-red-400 hover:border-red-400/40 hover:bg-red-400/10'
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-maroon to-gold text-white text-xs font-semibold hover:opacity-90 transition-opacity btn-press shadow-md shadow-maroon/20"
                    >
                ...
                {req.status === 'accepted' && (
                  <button className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-medium shrink-0 ${
              ...
              {/* Message */}
              <div className={`mt-4 p-3 rounded-xl text-sm ${isDark ? 'bg-void border-void-border border' : 'bg-light-bg border-light-border border'}`}>
